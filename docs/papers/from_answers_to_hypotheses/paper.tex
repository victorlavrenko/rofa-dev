...
We release a public, versioned snapshot of the complete codebase at
\url{https://github.com/victorlavrenko/rofa/releases/tag/paper/from-answers-to-hypotheses-v1}.

Reproduction is supported by two dedicated notebooks:

\begin{itemize}
  \item \textbf{Reproduction notebook} (analysis only):\\
  \path{notebooks/from_answers_to_hypotheses/20_paper_reproduce.ipynb}\\
  Reproduces all reported figures and statistical analyses from released
  model outputs, without rerunning model generation.

  \item \textbf{Generation notebook} (optional, stochastic):\\
  \path{notebooks/from_answers_to_hypotheses/10_colab_generate.ipynb}\\
  Performs end-to-end reproduction including stochastic model generation
  and must be executed prior to the reproduction notebook.
\end{itemize}
