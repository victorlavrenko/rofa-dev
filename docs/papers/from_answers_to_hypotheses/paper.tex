...
\texttt{HPAI-BSC/Llama3.1-Aloe-Beta-8B}, which has 8B parameters and is run in full
...
(\texttt{openlifescienceai/medmcqa}) using the official validation split. We
filter to single-choice questions with explanation length between 20 and 500
characters, then shuffle with seed 42 and select 400 questions with a per-subject
cap of 23 (20-subject balancing). This yields a deterministic 400-question
evaluation set.
...
The failure to find evidence supporting H3 motivates an alternative, empirically grounded interpretation for these errors:
...
Under this view, unanimous but incorrect predictions correspond to cases where the model's hypothesis space is locally coherent but globally misaligned with the task. This interpretation is not yet formally isolated from selection effects; the current analysis does not provide an operational test to conclusively separate missing hypotheses from low-probability selection. Such errors are therefore not reducible to stochastic selection noise alone and cannot be remedied by increased sampling or voting alone.
...
tested here. Together, these results underscore the analytical value of treating
...
The experiments use the \texttt{HPAI-BSC/Llama3.1-Aloe-Beta-8B} checkpoint (Llama
3.1 family, 8B parameters, instruction-tuned, open-weight, bfloat16) and the
MedMCQA validation split \parencite{pal2022medmcqa}
(\texttt{openlifescienceai/medmcqa}) with the deterministic 400-question selection
