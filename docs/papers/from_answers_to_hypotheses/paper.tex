...
\begin{enumerate}
  ...
  \item \textbf{Calibration/confidence errors} (e.g., 13.2\% wrong even when
    unanimous): internal agreement is misaligned with correctness.
  \item \textbf{Hypothesis-space limitations} (proxied by 58.6\% of errors where
    the gold answer is unsurfaced in the top-2): the model converges on an internally
    coherent but incorrect explanation under the tested sampling regime.
\end{enumerate}
