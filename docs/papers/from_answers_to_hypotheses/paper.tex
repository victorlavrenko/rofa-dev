...
\subsection{H1: Aggregation Improves Accuracy}
...
\subsection{H2: Correct Answers Appear Among Alternatives}
...
\subsection{H3: Internal Consensus Implies Correctness}
\label{sec:consensus}
...
\subsection{H4: Selective Leader Override Is Feasible}
\label{sec:h4}

We hypothesize that restricting override decisions to low-consensus regimes
substantially reduces the precision required of an override algorithm, while
retaining access to a large fraction of baseline errors.
...
Second, we measured the required suppression of false overrides, defined as the ratio between cases where the top-1 hypothesis is correct and cases where the top-2 hypothesis is correct within the regime. This ratio directly reflects how accurate a leader override decision policy must be in order to avoid degrading performance.
...
\begin{enumerate}
  \item Selection errors, where the correct hypothesis is present among alternatives but not selected as the final answer.
  \item Calibration or confidence errors, where internal agreement or confidence is misaligned with correctness.
  \item Hypothesis space errors, where the model converges on an internally coherent but systematically incorrect explanation.
\end{enumerate}
...
decision-making. We showed that aggregation improves accuracy (H1), that correct
answers frequently appear among alternative hypotheses (H2), and that internal
consensus strongly predicts correctness (H3). Building on these findings, we
demonstrated that selective leader override is feasible under conservative
uncertainty filtering, substantially reducing required override precision while
retaining meaningful error coverage. Together, these results support treating
reasoning as a distribution rather than a single trajectory.
