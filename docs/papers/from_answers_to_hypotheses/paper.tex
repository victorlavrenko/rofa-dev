...
\subsection{Limits of naive top-2 flipping}
...
\subsection{Flip feasibility and error budgets across uncertainty regimes}

To make this trade-off explicit, we analyzed the feasibility of selective top-2 flipping across vote-consensus regimes defined by top-1 vote counts and top-1/top-2 gaps. For each regime, we quantified two key quantities (Figure~\ref{fig:flip-feasibility}).

\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\linewidth]{figure2_top2_flip_feasibility}
    \caption{
    Feasibility of selective top-2 flipping across vote-consensus regimes.
    ...
    The y-axis indicates the required suppression of false flips (top-1 correct vs.\ top-2 correct).
...
We observe that moving the upper bound on top-1 vote counts from high-consensus regimes to uncertainty-focused regimes yields a more than twofold reduction in required false-flip suppression (from $\approx 2.4$ to $\approx 1.0$), a difference that is statistically significant ($p < 10^{-6}$).

\subsection{Implications for future flip algorithms}
...
\begin{itemize}
  \item explicit modeling of hypothesis sets and vote distributions rather than single predictions;
  \item learning flip decision policies that operate within predefined error budgets;
  \item training objectives that penalize confident convergence on incorrect hypotheses; and
  \item interfaces that expose structured uncertainty and alternative hypotheses to clinicians.
\end{itemize}
