...
1. Open `notebooks/from_answers_to_hypotheses/10_colab_generate.ipynb` in Colab.
...
2. Open `notebooks/from_answers_to_hypotheses/20_paper_reproduce.ipynb` and set the run paths.
...
Reports are written under `notebooks/from_answers_to_hypotheses/reports/<run_id>/report.json` by default.
...
4. Copy the asset URL into `notebooks/from_answers_to_hypotheses/20_paper_reproduce.ipynb` (asset URL field).
...
4. Analyze locally with `notebooks/from_answers_to_hypotheses/20_paper_reproduce.ipynb` or `scripts/analyze.py`.
